...
import {
  ...
  Heart, MessageSquare, Bell, Menu, X, Target, Trophy,
  Zap, Download, LogOut, Loader, RefreshCw, AlertCircle,
...
// ─── LEVEL CONFIG ─────────────────────────────────────────────────────────────
const LEVELS = [
  { key: 'beginner',     label: 'Beginner',     gigs: '0–5',   min: 0,  max: 5  },
  { key: 'volunteer',    label: 'Volunteer',     gigs: '6–15',  min: 6,  max: 15 },
  { key: 'regular',      label: 'Regular',       gigs: '16–30', min: 16, max: 30 },
  { key: 'professional', label: 'Professional',  gigs: '31–50', min: 31, max: 50 },
  { key: 'expert',       label: 'Expert',        gigs: '51+',   min: 51, max: 999 },
];
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
        ...
        <div className="p-6 border-t flex gap-3 justify-end">
          ...
          <button onClick={handleApply} disabled={loading}
            className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg font-semibold text-sm
              hover:opacity-90 transition-all active:scale-95 flex items-center gap-2 disabled:opacity-60">
...
// ─── JOB DETAIL MODAL ─────────────────────────────────────────────────────────
const JobDetailModal = ({ job, onClose, onApply, appliedJobIds }) => {
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-3">
            <img
              src={`https://i.pravatar.cc/150?u=${job.organizerId?._id}`}
              className="w-10 h-10 rounded-full"
              alt="Organizer"
            />
            <div>
              <p className="font-semibold text-gray-900">{job.organizerId?.fullName || 'Organizer'}</p>
              <p className="text-xs text-gray-500">Event Organizer • Verified</p>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {[
              ...
              { icon: Briefcase, label: 'Slots',     value: `${job.slotsTotal - job.slotsFilled} of ${job.slotsTotal} remaining` },
        ...
        <div className="p-6 border-t">
          {alreadyApplied ? (
            <div className="flex items-center gap-2 justify-center text-green-700 font-semibold">
              <CheckCircle size={20} />
              <span>You have already applied for this gig</span>
            </div>
          ) : (
            <button
              ...
              className="w-full py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-bold
                hover:opacity-90 transition-all active:scale-95 flex items-center justify-center gap-2">
...
const WorkerDashboard = () => {
  ...
  const [appliedJobIds,    setAppliedJobIds]    = useState(new Set()); // quick lookup
  ...
  const [applyingJob,      setApplyingJob]      = useState(null);  // job object for ApplyModal
  const [previewJob,       setPreviewJob]       = useState(null);  // job object for DetailModal
  ...
  const fetchDashboard = useCallback(async () => {
    setLoadingDash(true);
    const data = await apiFetch('/api/workers/dashboard');
    if (data.success) setDashboardData(data.data);
    else setErrorMsg(data.message || 'Failed to load dashboard.');
    setLoadingDash(false);
  }, []);

  // ── Fetch all active jobs (browse) ──────────────────────────────────────────
  ...
  const fetchMyApplications = useCallback(async () => {
    ...
    if (data.success) {
      ...
      // Build a Set of jobIds this worker already applied to
      const ids = new Set(data.data.applications
  ...
  // Re-fetch jobs when search/filter changes (debounced)
  useEffect(() => {
    const t = setTimeout(() => fetchJobs(searchQuery, filterCategory), 400);
    return () => clearTimeout(t);
  }, [searchQuery, filterCategory]);

  // Re-fetch apps when switching to applications tab
  useEffect(() => {
  ...
  const handleApplySuccess = (jobId) => {
    ...
    fetchMyApplications();
    fetchJobs(searchQuery, filterCategory);
    fetchDashboard();
  };
  ...
  const formatPay = (pay) => {
    if (!pay) return '—';
    const t = pay.type === 'per_day' ? '/day' : pay.type === 'per_hour' ? '/hr' : ' fixed';
    return `₹${pay.amount?.toLocaleString('en-IN')}${t}`;
  };
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8">
          ...
          {activeTab === 'browse' && (
            <div className="space-y-6">
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => fetchJobs(searchQuery, filterCategory)}
              ...
              {loadingDash ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {statsCards.map((stat, i) => (
                    <div key={i} className="group bg-white p-5 rounded-xl shadow-sm border border-gray-100
                      hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                      <div className="flex items-start justify-between mb-3">
                        <div className={`w-11 h-11 bg-gradient-to-r ${stat.color} rounded-lg flex items-center
                          justify-center text-white shadow-md group-hover:rotate-6 transition-transform`}>
                          <stat.icon size={22} />
                        </div>
                      </div>
              ...
              {loadingJobs ? (
                <div className="space-y-4">
                  {[...Array(3)].map((_, i) => <div key={i} className="bg-white rounded-xl h-40 border border-gray-100 animate-pulse" />)}
                </div>
              ) : availableJobs.length === 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-16 text-center">
                  <Search size={48} className="mx-auto text-gray-300 mb-3" />
                  <h3 className="text-lg font-bold text-gray-700 mb-1">No gigs found</h3>
                  <p className="text-gray-500 text-sm">Try a different search or check back later for new opportunities.</p>
                </div>
              ) : (
                <div className="grid gap-5">
                  {availableJobs.map(job => {
                    ...
                    return (
                      <div key={job._id}
                        className="group bg-white rounded-xl shadow-sm p-5 hover:shadow-xl transition-all duration-300
                          border-l-4 border-transparent hover:border-indigo-500 border border-gray-100 hover:-translate-y-0.5">
                        {job.urgent && (
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-start justify-between mb-2">
                              <div>
                                ...
                                <div className="flex items-center gap-2 text-sm text-gray-500 mt-0.5">
                                  <span className="font-medium">{job.organizerId?.fullName || 'Organizer'}</span>
                            ...
                            <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                              ...
                              <span className="flex items-center gap-1.5">
                                ...
                                {new Date(job.date).toLocaleDateString('en-IN')} • {job.time}
                              </span>
                              <span className="flex items-center gap-1.5">
                          ...
                          <div className="flex lg:flex-col gap-2 items-start">
                            {alreadyApplied ? (
                              ...
                              <button
                                ...
                                className="flex-1 lg:flex-initial px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600
                                  text-white rounded-lg font-semibold hover:brightness-110 hover:shadow-lg transition-all
                            ...
                            <button onClick={() => setPreviewJob(job)}
                              className="px-3 py-2.5 border border-gray-200 rounded-lg hover:bg-indigo-50 hover:text-indigo-600
                                hover:border-indigo-300 transition-all active:scale-90 text-sm flex items-center gap-1.5 font-medium">
                              <Eye size={15} /> View
                            </button>
                            <button
                              className="px-3 py-2.5 border border-gray-200 rounded-lg hover:bg-pink-50 hover:text-pink-600 transition-all active:scale-90"
                              title="Save">
                              <Heart size={16} />
                            </button>
                          </div>
            ...
            <div className="space-y-5">
              ...
              {loadingApps ? (
                <div className="space-y-3">
                  {[...Array(3)].map((_, i) => <div key={i} className="bg-white rounded-xl h-28 border border-gray-100 animate-pulse" />)}
                </div>
                ...
                <div className="grid gap-4">
                  {myApplications.map(app => {
                    ...
                    return (
                      <div key={app._id}
                        className="group bg-white rounded-xl shadow-sm p-5 border border-gray-100
                          ...
                          <div className="flex-1">
                            <div className="flex items-start justify-between mb-2">
                              <div>
                                ...
                                <p className="text-sm text-gray-500">{app.jobId?.organizerId?.fullName}</p>
                              </div>
                            ...
                            {app.coverNote && (
                              <p className="text-xs text-gray-500 italic bg-gray-50 px-3 py-2 rounded-lg mt-1">
                                "{app.coverNote}"
                              </p>
                            )}
                          </div>
                          ...
                          <div className="flex lg:flex-col gap-2 flex-shrink-0">
                            ...
                            {app.status === 'Pending' && (
                              <button
                                ...
                                className="px-4 py-2 border border-red-200 text-red-600 rounded-lg text-sm font-semibold
                                  hover:bg-red-50 transition-all active:scale-95 flex items-center gap-1.5 disabled:opacity-60">
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                            {job?.date && (
                              <span className="flex items-center gap-1.5">
                                ...
                                {new Date(job.date).toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                              </span>
                            )}
                            {job?.time && (
                              <span className="flex items-center gap-1.5">
          ...
          {activeTab === 'completed' && (
            <div className="space-y-5">
              ...
              {loadingDash ? (
                <div className="space-y-3">
                  {[...Array(3)].map((_, i) => <div key={i} className="bg-white rounded-xl h-32 border border-gray-100 animate-pulse" />)}
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-14 text-center">
                  ...
                  <p className="text-gray-500 text-sm">Complete your first gig to see it here with your rating and earnings</p>
                </div>
              ) : (
                <div className="grid gap-5">
                          ...
                          <div className="flex-1">
                            <div className="flex items-start justify-between mb-2">
                              <div>
                                ...
                                <p className="text-gray-500 text-sm">{app.jobId?.organizerId?.fullName}</p>
                              </div>
                              ...
                              <>
                                <div className="flex items-center gap-2 mb-2">
                                  <div className="flex">
                                    {[...Array(5)].map((_, i) => (
                                      <Star key={i} size={16}
                                        className={i < Math.floor(app.rating.score) ? 'text-yellow-500 fill-current' : 'text-gray-300'} />
                                    ))}
                                  </div>
                            ...
                            {job?.date && (
                              <p className="text-xs text-gray-400 mt-2">
                                Completed on {new Date(job.date).toLocaleDateString('en-IN')}
                              </p>
                            )}
                          </div>

                          <div>
                            <button className="px-5 py-2 border border-indigo-600 text-indigo-600 rounded-lg font-semibold text-sm
                              hover:bg-indigo-600 hover:text-white transition-all flex items-center gap-2">
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <p className="text-white/80 mb-4 capitalize">
                      ...
                      {dashboardData?.profile?.location?.city || 'Location not set'}
                    </p>
                    <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                      <div className="bg-white/10 px-4 py-2 rounded-lg backdrop-blur-sm hover:bg-white/20 transition-colors">
                        <p className="text-2xl font-bold">{dashboardData?.stats?.totalGigsCompleted || 0}</p>
                        <p className="text-xs text-white/70 uppercase">Completed Gigs</p>
                      </div>
                      <div className="bg-white/10 px-4 py-2 rounded-lg backdrop-blur-sm hover:bg-white/20 transition-colors">
                        <p className="text-2xl font-bold">{dashboardData?.stats?.averageRating ? dashboardData.stats.averageRating.toFixed(1) : '—'}</p>
                        <p className="text-xs text-white/70 uppercase">Avg Rating</p>
                      </div>
                      <div className="bg-white/10 px-4 py-2 rounded-lg backdrop-blur-sm hover:bg-white/20 transition-colors">
                        <p className="text-2xl font-bold">₹{(dashboardData?.stats?.totalEarnings || 0).toLocaleString('en-IN')}</p>
                        <p className="text-xs text-white/70 uppercase">Total Earned</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {dashboardData?.profile?.skills?.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {dashboardData.profile.skills.map(skill => (
                      <span key={skill} className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-full font-medium">{skill}</span>
                    ))}
                  </div>
                </div>
              )}

              {/* Badges — from DB or defaults */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div className="space-y-3">
                  {LEVELS.map((level, i) => {
                    ...
                    return (
                      <div key={level.key}
                        className={`flex items-center gap-4 p-3 rounded-xl transition-all ${isCurrent ? 'bg-indigo-50 shadow-sm' : 'hover:bg-gray-50'}`}>
                        <div className={`w-11 h-11 rounded-full flex items-center justify-center font-bold text-white transition-all ${
                          isCurrent ? 'bg-gradient-to-r from-indigo-600 to-purple-600 scale-110 shadow-lg' :
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-0.5">
                            <h4 className={`font-bold ${isCurrent ? 'text-indigo-600' : isPast ? 'text-green-600' : 'text-gray-500'}`}>
                              {level.label}
                            </h4>
                          ...
                          {isCurrent && nextLevelObj && (
                            <div className="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                              <div className="bg-gradient-to-r from-indigo-600 to-purple-600 h-1.5 rounded-full transition-all duration-1000"
            ...
            <div className="space-y-6">
              ...
              <div className="grid sm:grid-cols-3 gap-5">
                {[
                  ...
                  { label: 'Completed Gigs',  val: String(dashboardData?.stats?.totalGigsCompleted || 0),                            icon: CheckCircle, color: 'from-blue-500 to-blue-600'    },
                ].map((item, idx) => (
                  <div key={idx} className="group bg-white rounded-xl shadow-sm border border-gray-100 p-5
                    hover:shadow-lg transition-all hover:-translate-y-1">
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-11 h-11 bg-gradient-to-r ${item.color} rounded-lg flex items-center
                        justify-center text-white group-hover:rotate-6 transition-transform`}>
                        <item.icon size={22} />
                      </div>
              ...
              {/* Payment history — real completed gigs */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                {(dashboardData?.completedGigs || []).length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {(dashboardData?.completedGigs || []).map(app => (
                      <div key={app._id}
                        className="group flex items-center justify-between p-4 border border-gray-100 rounded-lg
                          hover:border-green-200 hover:bg-green-50/30 transition-all">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              {app.jobId?.date ? new Date(app.jobId.date).toLocaleDateString('en-IN') : ''}
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
      ...
      {previewJob && (
        <JobDetailModal
          ...
          onApply={job => { setPreviewJob(null); setApplyingJob(job); }}
